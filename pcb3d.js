...
(() => {
    ...
    const startAssembly = () => {
        ...
        if (isSmall()) {
            // roughly 2x larger than before (0.52 -> ~1.0), with a narrow-device guard
            const mobileBoardScale = window.innerWidth <= 360 ? 0.92 : 1.0;
            root.scale.setScalar(mobileBoardScale);
            root.position.set(0.16, -1.7, 0);
        ...
        const applyProgress = (p) => {
            ...
            if (isSmall()) {
                ...
                camera.position.z = lerp(12.8, 11.0, easeOutCubic(p));
                camera.position.y = lerp(6.45, 5.35, easeOutCubic(p));
                camera.lookAt(0.14, -1.42, 0);
            } else {
                camera.position.x = 0;
